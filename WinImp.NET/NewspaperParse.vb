...
Module NewspaperParse
	...
	Public ActivityCounts() As Short ' 2d array  first index specifies hour that activity occurred, second specifies country relations doing the activity
	
	Public Sub ParseNewsPaper(ByRef strLine As String)
		...
		If InStr(strLine, "-=[  EMPIRE NEWS  ]=-") > 0 Then
			npSection = enumNewspaperSections.NP_HEADER
			ReDim ActivityCounts(24, Nations.Count)
			Nations.ClearTelegramCount()
			Exit Sub
		ElseIf InStr(strLine, "===  Telecommunications  ===") > 0 Then 
			npSection = enumNewspaperSections.NP_TELECOMMUNICATIONS
			Exit Sub
		End If
		...
		Select Case npSection
			Case enumNewspaperSections.NP_TELECOMMUNICATIONS
				'     ===  Telecommunications  ===
